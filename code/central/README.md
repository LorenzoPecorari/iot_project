...
As you can see, there are multiple level of energy consumption. The first (more or less at 1300 mW) has been detected when central device start to initializate its elements. The next higher point is reached when the device starts to make the data sampling. When data average is communicated to the broker, there is the first spike (communication to internet consumes a lot of energy). After the communication, device goes to sleep and consumptions goes to 800 mW, saving more or less the 40% of energy. The highest point after sleep mode is required to wake up the system, but it is isolated. Device after waking up goes back to consume 1400 mW as in the previous sampling phase.
