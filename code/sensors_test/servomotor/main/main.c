...
//       width      width
//
//   * PERIOD = pw + sp, FREQUENCY = 1 / (pw + sp)
//   * DUTY CYCLE = pulse width * frequency =
//                = pulse width / (pulse width + space width)

#define S_PIN GPIO_NUM_1 // pin to change when all things have to be merged
...
void app_main(void){
    ...
    while(1){
        servo_movement();
        // xTaskCreate(servo_movement, "movement", 4096, NULL, 10, &taskHandler0);
